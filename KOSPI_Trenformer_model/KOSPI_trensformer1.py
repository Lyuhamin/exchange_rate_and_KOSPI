...
scaled_data[:, 1] *= 1.5  # KOSPI 종가에 가중치 부여
...
# Multi-Head Attention 추가 (깊이 개선)
for _ in range(4):  # 깊이 4층으로 설정
    attn_output = MultiHeadAttention(num_heads=8, key_dim=32, dropout=0.1)(x, x)
...
history = model.fit(X, y, epochs=200, batch_size=32, validation_split=0.2, verbose=1, callbacks=[early_stopping, reduce_lr])
