...
merged_df = merged_df.sort_values(by='Date').dropna()  # 날짜순 정렬 및 결측치 제거

# 데이터 스케일링
# MinMaxScaler를 사용해 각 컬럼을 0~1 범위로 정규화
scalers = {
...
for col, scaler in scalers.items():
    scaled = scaler.fit_transform(merged_df[[col]])  # 각 컬럼을 스케일링
    scaled_data.append(scaled)

scaled_data = np.concatenate(scaled_data, axis=1)  # 스케일링된 데이터를 합침
scaled_closing_price = scalers['Closing_Price'].transform(merged_df[['Closing_Price']])  # 종가만 따로 스케일링

# 모델 입력 준비 (Lookback 사용)
# 데이터를 입력으로 사용하고, 해당 날짜의 종가를 타겟으로 설정
lookback = 300
X, y = [], []
for i in range(lookback, len(scaled_data)):
    X.append(scaled_data[i-lookback:i])  # lookback만큼의 데이터를 입력으로 사용
    y.append(scaled_closing_price[i])   # 타겟은 종가
...
x = input_layer
for _ in range(3):  # Multi-Head Attention 레이어 3회 반복
    x = MultiHeadAttention(num_heads=8, key_dim=32)(x, x)
    x = LayerNormalization(epsilon=1e-6)(x)
    x = Dropout(0.3)(x)  # Dropout을 추가해 과적합 방지
x = GlobalAveragePooling1D()(x)  # Global Average Pooling 적용
x = Dense(256, activation='relu')(x)
x = Dropout(0.5)(x)  # Fully Connected Layer 후 Dropout 추가
